    ...
    try:
        ...
        for face in img1_detection:
            ...
            if (len(outnew)!=0):
                ...
                if (width<100):
                    cv2.putText(az1,ra, (x,y+10), cv2.FONT_HERSHEY_SIMPLEX,0.5, (0, 255, 0), 1, cv2.LINE_AA)
                else:
                    cv2.putText(az1, ra, (x, y + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            else:
                cv2.rectangle(az1, (x , y), (x + width, y + height), (255, 0, 0), 2)
                if (width < 100):
                    cv2.putText(az1, "Unknow", (x, y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
                else:
                    cv2.putText(az1, "Unknow", (x, y + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            del Distance[:]
            del Outold[:]
            del outnew[:]
            del Dis[:]
...
Label(window, text="Nhập K", font=("Arial Bold", 10)).pack(side=BOTTOM)
Button(window, text ="Thoát",width=10, command = out).pack(side=BOTTOM)
Button(window, text ="Bắt đâu",width=10, command = main).pack(side=BOTTOM)
Button(window, text ="Chọn ảnh",width=10, command = moanh).pack(side=BOTTOM)
window.mainloop()